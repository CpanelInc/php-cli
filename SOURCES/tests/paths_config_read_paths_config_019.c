/* ea-php-cli - tests/paths_config_read_paths_config_019.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "file_conf_mime_types=conf_mime_types\n" };
...
int main(int argc, char** argv) {
  ...
  strncpy(paths_config.file_conf_php_conf, "unset", 10);
  ...
  if (strncmp(paths_config.file_conf_mime_types, "conf_mime_types", 1024) != 0) {
    printf("ERROR: paths_config.file_conf_mime_types \"%s\" is not \"conf_mime_types\"\n", paths_config.file_conf_mime_types);
    return 1;
  } else {
    printf("  paths_config.file_conf_mime_types \"%s\" is set correctly to \"conf_mime_types\"\n", paths_config.file_conf_mime_types);
  }
  ...
  if (strnlen(paths_config.bin_httpd, 8) != 0) {
    printf("ERROR: paths_config.bin_httpd \"%s\" is not empty\n", paths_config.bin_httpd);
    return 1;
  } else {
    printf("  paths_config.bin_httpd \"%s\" is empty\n", paths_config.bin_httpd);
  }
