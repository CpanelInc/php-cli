/* ea-php-cli - tests/paths_config_read_paths_config_020.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "file_conf_srm_conf=conf_srm_conf\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.file_conf_srm_conf, "conf_srm_conf", 1024) != 0) {
    printf("ERROR: paths_config.file_conf_srm_conf \"%s\" is not \"conf_srm_conf\"\n", paths_config.file_conf_srm_conf);
    return 1;
  } else {
    printf("  paths_config.file_conf_srm_conf \"%s\" is set correctly to \"conf_srm_conf\"\n", paths_config.file_conf_srm_conf);
  }
