/* ea-php-cli - tests/paths_config_read_paths_config_021.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "file_conf_php_conf=conf_php_conf\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.file_conf_php_conf, "conf_php_conf", 1024) != 0) {
    printf("ERROR: paths_config.file_conf_php_conf \"%s\" is not \"conf_php_conf\"\n", paths_config.file_conf_php_conf);
    return 1;
  } else {
    printf("  paths_config.file_conf_php_conf \"%s\" is set correctly to \"conf_php_conf\"\n", paths_config.file_conf_php_conf);
  }
