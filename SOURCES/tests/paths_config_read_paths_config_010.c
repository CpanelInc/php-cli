/* ea-php-cli - tests/paths_config_read_paths_config_010.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "dir_modules=modules\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.dir_modules, "modules", 1024) != 0) {
    printf("ERROR: paths_config.dir_modules \"%s\" is not \"modules\"\n", paths_config.dir_modules);
    return 1;
  } else {
    printf("  paths_config.dir_modules \"%s\" is set correctly to \"modules\"\n", paths_config.dir_modules);
  }
