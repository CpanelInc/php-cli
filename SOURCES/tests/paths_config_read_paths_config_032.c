/* ea-php-cli - tests/paths_config_read_paths_config_032.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "dir_base=base#and stuff\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.dir_base, "base", 1024) != 0) {
    printf("ERROR: paths_config.dir_base \"%s\" is not \"base\"\n", paths_config.dir_base);
    return 1;
  } else {
    printf("  paths_config.dir_base \"%s\" is set correctly to \"base\"\n", paths_config.dir_base);
  }
