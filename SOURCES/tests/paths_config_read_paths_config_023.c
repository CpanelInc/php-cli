/* ea-php-cli - tests/paths_config_read_paths_config_023.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "bin_apachectl=apachectl\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.bin_apachectl, "apachectl", 1024) != 0) {
    printf("ERROR: paths_config.bin_apachectl \"%s\" is not \"apachectl\"\n", paths_config.bin_apachectl);
    return 1;
  } else {
    printf("  paths_config.bin_apachectl \"%s\" is set correctly to \"apachectl\"\n", paths_config.bin_apachectl);
  }
