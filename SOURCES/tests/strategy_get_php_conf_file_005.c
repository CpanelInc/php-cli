/* ea-php-cli - tests/strategy_get_php_conf_file_005.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  strncpy(paths_config.dir_conf, "/etc/somewhere", PATHS_CONFIG_DIR_CONF_SIZE);

  printf("testing strategy_get_php_conf_file with no cli_config set, and /etc/cpanel/ea4/php.conf is a directory, and paths_config.dirconf set to /etc/somewhere \n");
  ...
  if (strncmp(php_conf_file, "/etc/somewhere/php.conf.yaml", 1024) != 0) {
    printf("ERROR: php_conf_file \"%s\" is not \"/etc/somewhere/php.conf.yaml\"\n", php_conf_file);
    return 1;
  } else {
    printf("  php_conf_file \"%s\" is set correctly to \"/etc/somewhere/php.conf.yaml\"\n", php_conf_file);
  }
