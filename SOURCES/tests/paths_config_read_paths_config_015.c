/* ea-php-cli - tests/paths_config_read_paths_config_015.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "dir_docroot=docroot\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.dir_docroot, "docroot", 1024) != 0) {
    printf("ERROR: paths_config.dir_docroot \"%s\" is not \"docroot\"\n", paths_config.dir_docroot);
    return 1;
  } else {
    printf("  paths_config.dir_docroot \"%s\" is set correctly to \"docroot\"\n", paths_config.dir_docroot);
  }
