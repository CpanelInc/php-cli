/* ea-php-cli - tests/strategy_get_php_conf_file_003.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  strncpy(cli_config.ea_php_config, "/cli/provided/location", CLI_CONFIG_EA_PHP_CONFIG_SIZE);
  ...
  if (strncmp(php_conf_file, "/cli/provided/location", 1024) != 0) {
    printf("ERROR: php_conf_file \"%s\" is not \"/cli/provided/location\"\n", php_conf_file);
    return 1;
  } else {
    printf("  php_conf_file \"%s\" is set correctly to \"/cli/provided/location\"\n", php_conf_file);
  }
