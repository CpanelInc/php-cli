/* ea-php-cli - tests/paths_config_read_paths_config_006.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "one=1", "two=2", "three=3" };
...
int main(int argc, char** argv) {
  ...
  if (fgets_call_count != 4) {
    printf("ERROR: fgets called %d times, not once\n", fgets_call_count);
    return 1;
  } else {
    printf("  fgets called %d times, or once\n", fgets_call_count);
  }
