/* ea-php-cli - tests/paths_config_read_paths_config_018.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.file_conf, "conf", 1024) != 0) {
    printf("ERROR: paths_config.file_conf \"%s\" is not \"conf\"\n", paths_config.file_conf);
    return 1;
  } else {
    printf("  paths_config.file_conf \"%s\" is set correctly to \"conf\"\n", paths_config.file_conf);
  }
