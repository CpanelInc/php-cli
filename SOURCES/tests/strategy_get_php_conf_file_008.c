...
int main(int argc, char** argv) {
  ...
  memset(paths_config.dir_conf, 0, PATHS_CONFIG_DIR_CONF_SIZE);

  printf("testing strategy_get_php_conf_file with no cli set, no /etc/cpanel/ea4/php.conf, and paths_config.dir_conf set to nothing\n");
  ...
  if (strncmp(php_conf_file, "/php.conf.yaml", 1024) != 0) {
    printf("ERROR: php_conf_file \"%s\" is not \"/php.conf.yaml\"\n", php_conf_file);
    return 1;
  } else {
    printf("  php_conf_file \"%s\" is set correctly to \"/php.conf.yaml\"\n", php_conf_file);
  }
