/* ea-php-cli - tests/paths_config_read_paths_config_024.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.bin_suexec, "suexec", 1024) != 0) {
    printf("ERROR: paths_config.bin_suexec \"%s\" is not \"suexec\"\n", paths_config.bin_suexec);
    return 1;
  } else {
    printf("  paths_config.bin_suexec \"%s\" is set correctly to \"suexec\"\n", paths_config.bin_suexec);
  }
