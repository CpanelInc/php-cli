/* ea-php-cli - tests/paths_config_read_paths_config_014.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "dir_conf_userdata=userdata\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.dir_conf_userdata, "userdata", 1024) != 0) {
    printf("ERROR: paths_config.dir_conf_userdata \"%s\" is not \"userdata\"\n", paths_config.dir_conf_userdata);
    return 1;
  } else {
    printf("  paths_config.dir_conf_userdata \"%s\" is set correctly to \"userdata\"\n", paths_config.dir_conf_userdata);
  }
