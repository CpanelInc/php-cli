/* ea-php-cli - tests/paths_config_read_paths_config_016.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_fgets(char* line, size_t size, FILE* file) {
   printf("fgets, call %d\n", fgets_call_count);
   char* lines[] = { "file_access_log=access_log\n" };
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.file_access_log, "access_log", 1024) != 0) {
    printf("ERROR: paths_config.file_access_log \"%s\" is not \"access_log\"\n", paths_config.file_access_log);
    return 1;
  } else {
    printf("  paths_config.file_access_log \"%s\" is set correctly to  \"access_log\"\n", paths_config.file_access_log);
  }
