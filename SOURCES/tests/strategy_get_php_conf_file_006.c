/* ea-php-cli - tests/strategy_get_php_conf_file_006.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  printf("testing strategy_get_php_conf_file with no cli_config set, /etc/cpanel/ea4/php.conf is not readable, and paths_config.dirconf set to /etc/somewhere \n");
  ...
  if (strncmp(php_conf_file, "/etc/somewhere/php.conf.yaml", 1024) != 0) {
    printf("ERROR: php_conf_file \"%s\" is not \"/etc/cpanel/php.conf.yaml\"\n", php_conf_file);
    return 1;
  } else {
    printf("  php_conf_file \"%s\" is set correctly to \"/etc/cpanel/php.conf.yaml\"\n", php_conf_file);
  }
