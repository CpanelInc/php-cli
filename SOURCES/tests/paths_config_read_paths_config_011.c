/* ea-php-cli - tests/paths_config_read_paths_config_011.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  if (strncmp(paths_config.dir_run, "run", 1024) != 0) {
    printf("ERROR: paths_config.dir_run \"%s\" is not \"run\"\n", paths_config.dir_run);
    return 1;
  } else {
    printf("  paths_config.dir_run \"%s\" is set correctly to \"run\"\n", paths_config.dir_run);
  }
