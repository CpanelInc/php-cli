/* ea-php-cli - tests/cli_config_read_cli_config_012.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int __wrap_yaml_parser_parse(yaml_parser_t* parser, yaml_event_t* event) {
    ...
    yaml_char_t* event_values[] = { 0, 0, 0, (yaml_char_t*)"testing_root_dir",
...
int main(int argc, char** argv) {
  ...
  printf("testing cli_config_read_cli_config on regular file, containing one document containing an mapping for testing_root_dir\n");
  ...
  if (strncmp(cli_config.testing_root_dir, "my_value", 1024) != 0) {
    printf("ERROR: cli_config.testing_root_dir \"%s\" is not \"my_value\"\n", cli_config.testing_root_dir);
    return 1;
  } else {
    printf("  cli_config.testing_root_dir \"%s\" is set correctly to \"my_value\"\n", cli_config.testing_root_dir);
  }
