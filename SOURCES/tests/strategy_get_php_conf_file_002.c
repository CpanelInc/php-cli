/* ea-php-cli - tests/strategy_get_php_conf_file_002.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int main(int argc, char** argv) {
  ...
  struct paths_config* paths_config = 0;
  char php_conf_file[1024] = "junk";

  printf("testing strategy_get_php_conf_file on null cli_config\n");
  printf("  calling strategy_get_php_conf_file(\"%s\", %d, &cli_config, (paths_config*)0\n", php_conf_file, 1024);
  ...
  if (strnlen(php_conf_file, 1024) != 0) {
    printf("ERROR: php_conf_file \"%s\" is not empty\n", php_conf_file);
    return 1;
  } else {
    printf("  php_conf_file \"%s\" is empty\n", php_conf_file);
  }
