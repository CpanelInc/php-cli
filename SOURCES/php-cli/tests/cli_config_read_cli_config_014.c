/* ea-php-cli - tests/cli_config_read_cli_config_014.c  Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
int __wrap_yaml_parser_parse(yaml_parser_t* parser, yaml_event_t* event) {
    ...
    yaml_char_t* event_values[] = { 0, 0, 0, (yaml_char_t*)"lsphp_bin_pattern",
...
int main(int argc, char** argv) {
  ...
  strncpy(cli_config.lsphp_bin_pattern, "unset", 10);

  int expected_yaml_parser_parse_called_count = 8;

  printf("testing cli_config_read_cli_config on regular file, containing one document containing an mapping for lsphp_bin_pattern\n");
  ...
  if (strncmp(cli_config.lsphp_bin_pattern, "my_value", 1024) != 0) {
    printf("ERROR: cli_config.lsphp_bin_pattern \"%s\" is not \"my_value\"\n", cli_config.lsphp_bin_pattern);
    return 1;
  } else {
    printf("  cli_config.lsphp_bin_pattern \"%s\" is set correctly to \"my_value\"\n", cli_config.lsphp_bin_pattern);
  }
