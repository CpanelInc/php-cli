/* ea-php-cli - tests/path_php_cli_yaml_005.c   Copyright 2016 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
char* __wrap_readlink(char* link, char* buf, size_t size) {
    char* return_value = 0;
    if (size > strlen("/usr/local/bin/lsphp")) {
        return_value = strncpy(buf, "/usr/local/bin/lsphp", size);
        if (return_value) {
            buf[size-1] = 0;
        }
    }
...
int main(int argc, char** argv) {
  char path[100] = { 0 };
  char* return_value = 0;
  char* expected_path = "/usr/local/bin/php_cli.yaml";
