/* ea-php-cli - src/php-cgi.c                   Copyright 2017 cPanel, Inc. */
/*                                                     All rights Reserved. */
...
void get_bin_php_default_pattern(char* buffer, size_t size) {
    strncpy(buffer, "%s/%s/root/usr/bin/php-cgi", size);
}
...
int main(int argc, char* argv[]) {
    ...
    if (php_bin[0] == 0) {
        if (php_package[0] != 0) {
            fprintf(stderr, "Configured PHP version %s does not have a CGI binary installed, using default PHP version\n", php_package);
        }
        strncpy(php_package, get_default_php_package(php_conf_file), 20);
        strategy_get_php_bin(php_bin, 1024, &cli_config, php_package);
    }
    if (php_bin[0] == 0) {
        fprintf(stderr, "Default PHP version %s does not have a CGI binary installed, exiting\n", php_package);
    }
